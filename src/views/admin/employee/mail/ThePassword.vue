<template>
    <nav class="nav nav-pills nav-justified mb-3">
        <router-link :to="{ name: 'admin:employee:mail:profile', params: { id: mailId } }"
...
export default defineComponent({
    ...
    setup() {
        const { localEmployeeMail, updatePassword } = useAdminEmployeeMail();

        const route = useRoute();

        const formErrors = computed(() => {
            return localEmployeeMail.formErrors;
        });
        ...
        const formSuccess = computed(() => {
            return localEmployeeMail.formSuccess;
        });
